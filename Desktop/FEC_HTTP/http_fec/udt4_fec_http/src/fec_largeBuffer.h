...
public:
   ...
   twoDXORPacket*** m_RcvXOR;                 // the list of XOR packets that has been recieved for loss recovery
   CPacket m_RecoveredPkt;             // the recovered packet
   bool* m_busyThreads;
...
public:
   ...
   int* findIndexInsert(int32_t seqNo);
   virtual void onPktSent(CPacket* packet);
   virtual void onPktReceived(const CPacket* packet);
   virtual void processCustomMsg(const CPacket* packet);
   virtual void lossRecovery(int row_index, int col_index, bool row);
   void renewCurrentAndNext();
   void writeRedundantPktFile(twoDXORPacket* m_SndXOR);
   void writeRedundantPktFile(char* XOR, int size);
   /*virtual void lossDetection();
   virtual void lossDetection(twoDXORPacket* XOR);
   virtual void lossDetection(const CPacket* packet);
   virtual void lossReport(CPacket*** list, int32_t firstSeq, bool current);*/
   virtual void lossRecoveryInvestigation (int row_index, int col_index, bool row);
...
protected:
   ...
   int *m_rowSize;
   int *m_colSize;
   CPacket*** m_buffer;               // Array of current FEC window
   int m_bufferSize;
   int m_bufferRowNum;
