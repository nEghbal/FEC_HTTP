...
void twoDXORFEC::onPktSent(CPacket* packet)
{
   ...
   //cout << "row " << m_sndIndices[0] << "    col " << m_sndIndices[1] << endl << std::flush;

   // doing XOR of header for row
   if (m_sndIndices[1] == 0)
   ...
   if (m_sndIndices[1] == m_col_FECRate - 1) {
      m_XORFile >> XOR_len;
      cout << "row " << m_sndIndices[0] << "   XOR_len:   " << XOR_len << endl << std::flush;
      XOR = new char[XOR_len + CPacket::m_iPktHdrSize];
      memcpy(XOR, (char *)(m_SndXOR[m_sndIndices[0]]->m_XORHeader), CPacket::m_iPktHdrSize);
      m_XORFile.read(XOR+CPacket::m_iPktHdrSize, XOR_len);
      writeRedundantPktFile(XOR+CPacket::m_iPktHdrSize, XOR_len);
      //m_UDT->sendRedundantPkt(XOR, XOR_len + CPacket::m_iPktHdrSize); // send an XOR*/
   }
   // doing XOR of header for col
   if (m_sndIndices[0] == 0)
   ...
   if (m_sndIndices[0] == m_row_FECRate - 1 && m_sndIndices[1] == m_col_FECRate - 1) {
      for (int i = 0; i < m_col_FECRate; ++ i) {
         m_XORFile >> XOR_len;
         cout << "col " << i << "   XOR_len:   " << XOR_len << endl << std::flush;
         XOR = new char[XOR_len + CPacket::m_iPktHdrSize];
