...
void twoDXORFEC::init2()
{
   ...
   m_col_current = new CPacket** [m_col_FECRate];
...
void* XOR(void *input) 
{
   ...
   //cout << " Thread: start     " << pthread_self() << endl << std::flush;
   for (int i = 0; i < arg->size; ++ i) {
      while (i > arg->index)
         ;
      cout << " arg->index:  "<<arg->index<<"   " << pthread_self() << endl << std::flush;
      //cout << " Thread: middle     " << pthread_self() << endl << std::flush;
      if (i == 0) 
   ...
   delete [] arg->list;
   cout << " Thread: end     " << pthread_self() << endl << std::flush;
   arg->end = true;
 }

void twoDXORFEC::XORProcessing(CPacket* packet, CPacket** list, bool row) 
{
   int index1, index2, size, index3;
   if (row) {
      index1 = m_sndIndices[1];
      index2 = m_sndIndices[0];
      size = m_col_FECRate;
      index3 = m_sndIndices[0];
   }
   else {
      index1 = m_sndIndices[0];
      index2 = m_sndIndices[1];
      size = m_row_FECRate;
      index3 = m_row_FECRate+m_sndIndices[1];
   } 
   list[index1] = new CPacket();
   list[index1]->m_pcData = new char[m_UDT->m_iPayloadSize];
   insert(packet, list, index1);
   if (index1 > 0 && m_busyThreads[index3] == true) {
      thread_args[index3]->index ++;
   }
   if (index1 == 0) {
      if (m_busyThreads[index3] == false) { 
         thread_args[index3]->list = list;
         thread_args[index3]->size = size;
         thread_args[index3]->row = row;
         thread_args[index3]->index = 0;
         thread_args[index3]->end = false;
         thread_args[index3]->XOR = m_SndXOR[index3];
         m_busyThreads[index3] = true;
         pthread_create(&(filethread[index3]), NULL, XOR, thread_args[index3]);
         cout << " Thread: create " << index3 << "   " << filethread[index3] << endl << std::flush;
         pthread_detach(filethread[index3]);
      }
      else {
         if (row)
            cout << "cannot create thread for row "<< m_sndIndices[0] << endl << std::flush;
         else
            cout << "cannot create thread for col "<< m_sndIndices[1] << endl << std::flush;
         delete [] list[0]->m_pcData;
         delete list[0];
         delete [] list;
      }
   }
...
void twoDXORFEC::onPktSent(CPacket* packet)
{
   ...
   /*if (m_col_current[m_sndIndices[1]] == NULL)
      m_col_current[m_sndIndices[1]] = new CPacket*[m_row_FECRate];
   XORProcessing(packet, m_col_current[m_sndIndices[1]], false);
   if (m_sndIndices[0] == m_row_FECRate - 1)
      m_col_current[m_sndIndices[1]] = NULL;
   if (m_busyThreads[m_row_FECRate+current_xor_col] == true) {
      if (thread_args[m_row_FECRate+current_xor_col]->end == true) {
         cout << "col  index = " << current_xor_col << endl << std::flush;
