...
{
   //cout << m_sndIndices[0] << "  " << m_sndIndices[1] << "   " << m_totalCurrentSize << endl << std::flush;
   ...
   while (m_busyThreads[m_row_FECRate+current_xor_col] == true) {
      if (thread_args[m_row_FECRate+current_xor_col]->end == true) {
         ...
         if (current_xor_col == m_col_FECRate) {
            current_xor_col = 0;
            //cout << "current xor row: " << current_xor_row << endl << std::flush;
            if (current_xor_row == 0) {
               //if (m_sndNextSize > 0)
               //   cout << "Next: " << m_sndNextSize << endl << std::flush;
               sndRenewCurrentAndNext();
   ...
   /*for (int i = 0; i < m_row_FECRate+m_col_FECRate; ++ i) {
      if (m_busyThreads[i] == true) 
         if (thread_args[i]->end == true) {
            cout << "i = " << i << endl << std::flush;
            m_UDT->sendRedundantPkt(m_SndXOR[i]);
            m_busyThreads[i] = false;
         }
   }*/
...
{
   if (current) {
      if (row_index >=0 && m_row_currentSize[row_index] == m_col_FECRate - 1 && m_RcvXORCurrent[row_index] != NULL &&
          ((row_index < m_row_FECRate - 1 && m_row_currentSize[row_index+1] > 0) || 
          (row_index == m_row_FECRate - 1 && m_totalNextSize > 0))) 
         lossRecovery(row_index, true);
      
      if (col_index >=0 && m_col_currentSize[col_index] == m_row_FECRate - 1 &&  m_RcvXORCurrent[m_row_FECRate+col_index] != NULL && m_totalNextSize > 0) 
            lossRecovery(col_index, false);
   } 
   else {
      if (row_index >=0 && m_row_nextSize[row_index] == m_col_FECRate - 1 && m_RcvXORNext[row_index] != NULL &&
          ((row_index < m_row_FECRate - 1 && m_row_nextSize[row_index+1] > 0) ||
          (row_index == m_row_FECRate - 1))) 
         lossRecoveryNext(row_index);
   }
}
...
{   
   ...
   if (CSeqNo::seqcmp(newXOR->m_firstSeq, CSeqNo::decseq(m_firstSeqNext)) > 0 && 
       CSeqNo::seqcmp(newXOR->m_firstSeq, CSeqNo::incseq(m_firstSeqNext, m_row_FECRate*m_col_FECRate)) < 0 &&
       newXOR->m_row == false) {
      //cout << "2  row: "<< newXOR->m_row << "   newXOR->m_firstSeq: " << newXOR->m_firstSeq << "   firstSeqNext: " << m_firstSeqNext << endl << std::flush;
      lossDetection(newXOR);
   }
   else if (CSeqNo::seqcmp(newXOR->m_firstSeq, CSeqNo::decseq(m_firstSeqNext)) > 0 && 
       CSeqNo::seqcmp(newXOR->m_firstSeq, CSeqNo::incseq(m_firstSeqNext, m_row_FECRate*m_col_FECRate)) < 0 &&
       newXOR->m_row == true) {
      //cout << "3  row: "<< newXOR->m_row << "   newXOR->m_firstSeq: " << newXOR->m_firstSeq << "   firstSeqNext: " << m_firstSeqNext << endl << std::flush;
      indices = findIndexInsert(m_firstSeqNext, newXOR->m_firstSeq);
      if (indices[0] >= 0) {
         m_RcvXORNext[indices[0]] = newXOR;
      ...
      if (indices[0] >= 0) {
         if (newXOR->m_row) {
            m_RcvXORCurrent[indices[0]] = newXOR;
            lossRecoveryInvestigation (indices[0], -1, true); 
         }
         else { 
            m_RcvXORCurrent[m_row_FECRate + indices[1]] = newXOR;
...
{  
   ...
   if (row) {
      ...
      if (m_totalCurrentSize == m_row_FECRate*m_col_FECRate)
         ...
         lossRecoveryInvestigation (-1, lostPktIndex, true);
   }
   else {
...
void twoDXORFEC::renewCurrentAndNext(bool empty_next) {
   //if (m_totalCurrentSize < m_row_FECRate*m_col_FECRate)
   //   cout << m_totalCurrentSize << endl << std::flush;
   if (empty_next) {
