...
void twoDXORFEC::init2()
{
   ...
   m_rowSize = new int[buffer_factor*m_row_FECRate];
   m_colSize = new int[buffer_factor*m_col_FECRate];
   ...
   for (int i = 0; i < buffer_factor*m_col_FECRate; ++ i) 
      m_colSize[i] = 0;
   ...
   //m_logFile.open ("/home/eghbal/udt4_fec/logFile.txt");
   m_XORFile.open ("/dev/shm/eghbal/XOR_1.txt");
   m_XORFile2.open ("/dev/shm/eghbal/XOR_2.txt");
   //fstream m_XORFile("/dev/shm/eghbal/XOR.txt", ios::in | ios::binary);
}
   ...
   if (m_sndIndices[0] == m_row_FECRate - 1 && m_sndIndices[1] == m_col_FECRate - 1) {
      for (int i = 0; i < m_col_FECRate; ++ i) {
         ...
         if (XOR_len == 8981) {
            XOR = new char[XOR_len];
            m_XORFile.read(XOR, XOR_len);
            memcpy(XOR, (char *)(m_SndXOR[m_row_FECRate+i]->m_XORHeader), CPacket::m_iPktHdrSize);
            /*writeRedundantPktFile(XOR, XOR_len);*/
            m_UDT->sendRedundantPkt(XOR, XOR_len); 
         }
      }
...
{     
   ...
   if (indices[0] >= 0) {
      ...
      m_rowSize[indices[0]] ++;
      m_colSize[(indices[0]/m_row_FECRate)*m_col_FECRate + indices[1]] ++;
      m_bufferSize ++;
...
{
   if (row && m_rowSize[row_index] == m_col_FECRate - 1 && m_RcvXOR[row_index/m_row_FECRate][row_index%m_row_FECRate] != NULL) {
      //cout << "row recovery! 1" << endl << std::flush;
      if (row_index < m_row_FECRate*buffer_factor - 1 && m_rowSize[row_index+1] > 0 || row_index == m_row_FECRate*buffer_factor - 1 && m_rowSize[0] > 0)
         //cout << "row recovery! 2" << endl << std::flush;
         lossRecovery(row_index, col_index, true);
   }

   if (col_index > -1 && m_colSize[(row_index/m_row_FECRate)*m_col_FECRate + col_index] == m_row_FECRate - 1 && 
       m_RcvXOR[row_index/m_row_FECRate][m_row_FECRate+col_index] != NULL) {
      //cout << "col recovery! 1" << endl << std::flush;
      if (row_index < m_row_FECRate*(buffer_factor-1) && m_rowSize[((row_index/m_row_FECRate)+1)*m_row_FECRate] > 0 || 
          row_index == m_row_FECRate*(buffer_factor-1) && m_rowSize[0] > 0)
         //cout << "col recovery! 2" << endl << std::flush;
         lossRecovery(row_index, col_index, false);
   }
}

void twoDXORFEC::writeRedundantPktFile(char* XOR, int size)
{
   m_XORFile2 << size;
   cout << "write: "<< size << endl << std::flush;
   m_XORFile2.write(XOR, size);
   m_XORFile2.flush();
}
...
{   
   ...
   //writeRedundantPktFile(XOR->m_pcData+CPacket::m_iPktHdrSize, XOR->getLength()- CPacket::m_iPktHdrSize);
...
void twoDXORFEC::lossRecovery(int row_index, int col_index, bool row)  // Update rowSize & colSize!!!
{  
   int lostPktIndex = -1;
   if (row) {
      for (int j = 0; j < m_col_FECRate; ++ j) {
      ...
      m_bufferSize ++;
      m_rowSize[row_index] ++;
      m_colSize[(row_index/m_row_FECRate)*m_col_FECRate +lostPktIndex] ++;
      ...
      m_bufferSize ++;
      m_rowSize[lostPktIndex] ++;
      m_colSize[(lostPktIndex/m_row_FECRate)*m_col_FECRate +col_index] ++;
...
void twoDXORFEC::renewCurrentAndNext() {
   ...
   for (int i= m_current_buffer*m_row_FECRate; i < (m_current_buffer+1)*m_row_FECRate; ++ i) {
      m_rowSize[i] = 0;
      if (i == (m_current_buffer+1)*m_row_FECRate - 1 && m_current_buffer < buffer_factor - 1)
         m_rowSize[i+1] = 0;
      else if (i == (m_current_buffer+1)*m_row_FECRate - 1 && m_current_buffer == buffer_factor - 1)
         m_rowSize[0] = 0;
      for (int j= 0; j < m_col_FECRate; ++ j) {
         if (i == m_current_buffer*m_row_FECRate)
            m_colSize[m_current_buffer*m_col_FECRate+j] = 0;
